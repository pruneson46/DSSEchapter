...
In this chapter, we support the \emph{design science paradigm} as a frame for software engineering, as earlier proposed by Wieringa \cite{wieringa_what_2014} which we merge with perspectives from van Aken \cite{van_aken_management_2004}. The design science paradigm integrates \emph{problem understanding, solution design} and \emph{validation}. This paradigm fits well as a frame for empirical software engineering with its aim to provide practical solutions for real-world software engineering challenges. In particular, multiple case studies are proposed as the typical research methodology in design science \cite{van_aken_management_2004}, which aligns with the widespread use of case studies in software engineering.
...
%\peggy{I wonder if "transitions across" may be better than "movements"?} \per{Definitely agree. I considered that, but was not sure if that was proper English. }
...
\item \emph{Solution design} refers to the activity of mapping a problem to a general solution, \item \emph{Instantiation} refers to the activity of implementing the solution in context; and 
...
\peggy{we have a mix of north american and british english -- changing to north american but maybe you prefer british? I can change back if you like!} \per{I propose we go for north american}
...
This recommendation fits well with the design science paradigm and its pragmatists viewpoint.
...
Design science may embrace a multitude of research methods to be used. For \emph{problem understanding} and \emph{validation} of technological rules, empirical research methods are used. Methods supporting realism and natural settings~\cite{StolABC18} are preferred as the problem in context is in focus for design science research. 
...
Gregor and Hevner also refer to design science as a paradigm~\cite{gregor_positioning_2013}. Johannesson, on the other hand, disagrees with this and argues that design science refers to the objective of changing the world, in contrast to describing it, and that this is done primarily by creating artifacts not knowledge~\cite{johannesson_introduction_2014}. Johannesson's view emphasizes design science as action research. Our stance is that design science is a paradigm for software engineering research and as researchers our primary goal is to create knowledge to be applied by practitioners in the field. Furthermore, we consider artifacts as embedding design knowledge. 
