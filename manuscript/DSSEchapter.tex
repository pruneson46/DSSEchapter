...
The design science paradigm integrates\emelie{comprises?} \emph{problem conceptualization, solution design} and \emph{validation}. In this chapter, we demonstrate how this paradigm fits as a frame for empirical software engineering with its aim to provide knowledge about practical solutions\emelie{what about the design propositions? This sentence (and maybe the whole paragraph) put my thoughts on building things rather than building theory}\per{added: knowledge about...solutions} for real-world software engineering challenges. In particular, multiple case studies are proposed as the typical research methodology to gain design knowledge under the design science paradigm~\cite{van_aken_management_2004}, which aligns with the widespread use of case studies in software engineering. 
...
\caption{A visualization of the different types of research activities that take place in design science research. These activities may be instantiated in different ways.}
...
Below we explore the contributions and activities of the design science model. 
...
In a mature research field, existing theory may help practitioners design solutions to their own specific problems. Problem conceptualization is then an act of the practitioner, as well as the instantiation of the solution in a specific context. In fields where the theoretical foundation is less mature, such as software engineering, researchers and practitioners work together to advance and extend the scope of the theory. \emelie{I think it is ok, but wonder if we are aware of such mature field :-). I guess researchers may be part of advancing any field of research} \per{I also doubt, but I think this way of expressing it as a matter of maturity is clearer than writing about an ideal situation.}
...
The outcome of the problem conceptualization is expressed in terms of problem elements/constructs, matching coresponding constructs of an envisioned solution. If, for example, the proposed solution is to design a visualization system, the problem should typically be described in terms of a group of target users, their questions and tasks, and their measurements or data~\cite{meyer_nested_2015}. Thus, the problem formulation is tightly connected to the solution design and cannot be done in isolation. 
...
The \emph{relevance} of the research is ensured \emelie{Relevance is a subjective measure so I suggest to reformulate either by telling to whom relevance is ensured in this case or by saying that we ensure that relevance may be assessed} by anchoring the research in the literature as well as in the specific problem instances of the study. \per{Proposed change: \emph{Relevance} to the practitioner is ensured by anchoring the research in a specific problem instance of the study, while \emph{relevance} to the researcher is ensured by connecting to existing literature.}
...
Referring to the example by Jonsson \emph{et al.}, applying machine learning to bug assignment is  a frequent topic of research in software engineering. These authors report 20 previous studies on machine learning-based bug assignment \cite[Fig.2]{JonssonBug15}, which adds evidence that this problem is relevant in practice.\emelie{this example is not inline with the text. Instead the relevant context factors for doing this assessment should be reported. I'm not sure that the number of previous studies in it self helps the practitioner to judge wether the solution is applicable or will have effect in his/her context. A high number of ML studies may give strength to the proposition, while a high number of bug assignment studies may indicate general relevance. But I would say it depends on the variety of the studies (a high number of stakeholders and different problem instances is maybe a better way to measure general relevance. And listing common denominators of the problem instances together with constraints of the general solution is maybe att better way to guide the practitioner's relevance assessment)} \per{Rephrase: These authors report 20 previous studies on machine learning-based bug assignment, with different models to various bug report sets \cite[Fig.2]{JonssonBug15}, which adds evidence that this problem is relevant in practice.}
...
It is worth noting that the solution design activity is by nature a creative process and does not necessarily have to add to the rigor of the research overall. One aspect of rigor in the design activity could be the extent to which a solution is built on prior design knowledge, or whether alternative solutions have been taken into account. 
...
For example, Jonsson \emph{et al.}'s research on automated bug assignment is related to previous bug assignment research, although it is hard to compare due to lack of detail and non-standardized validation practice \emelie{I agree with Peggy that lack of validation (though it leads to weak propositions) is not the main problem but rather the inconsistent generalization of contributions}.\per{Yes, generalization is the key, but if different instances are validated in different ways (e.g. using different evaluation metrics), it is impossible to generalize} If the contributions were clearly expressed as design propositions with corresponding validations, the outcomes together would be more generalizable. 
...
Van Aken explored the design science paradigm for management science, with a focus on theoretical contributions, captured as design propositions\emelie{should we mention that he calls it technological rules?} \cite{van_aken_management_2004,van_aken_management_2005}. 
